...
long long traverse(int cur){
    ...
    if (tree[cur].is_file){
        ...
        for (int subdir : tree[cur].child_list){
            output += traverse(subdir);
        }
    }
    return tree[cur].dirsize = output;
}

int main(){
    initialize();
    parse_command(dir_ptr);
    long long total = traverse(0);
    long long free = 70000000 - total;
    long long output = 1e16 + 7;
    for (int i = 0; i < top; i++){
        if (!tree[i].is_file){
            if (free + tree[i].dirsize >= 30000000){
                output = min(output, tree[i].dirsize);
            }
        }
    }
    fo << output;
